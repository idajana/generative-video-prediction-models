...
parser.add_argument('--params', default="params/eval_baseline.yaml", metavar='params',
...
tmp = torch.load(eval_params["model_path"])
params = tmp["params"]
...
def make_gifs(x, idx, name, action, plot):
    ...
    for i in range(1, n_eval):
        ...
        h = h.detach()
        if i < n_past:
            frame_predictor(torch.cat([h, action[:,i-1]], 1))
            posterior_gen.append(x[:,i])
            x_in = x[:,i]
        else:
            h_pred = frame_predictor(torch.cat([h, action[:,i-1]], 1)).detach()
    ...
    for s in range(nsample):
        ...
        for i in range(1, n_eval):
            ...
            if i < n_past:
                ...
                h = frame_predictor(torch.cat([h, action[:,i-1]], 1)).detach()
