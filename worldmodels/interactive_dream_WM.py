"""
Simulated Worldmodels environment.
"""
import argparse
...
from worldmodels.MD_RNN import MDRNNCell
LSIZE = 128
RED_SIZE = 64
RSIZE = 512
ACTION_SIZE =6
...
class SimulatedCarracing(gym.Env): # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, directory):
        ...
        # load VAE
        self.vae = VAE(nc = 3, ngf=64, ndf=64, latent_variable_size=128, cuda=False)
        ...
        # load MDRNN
        self._rnn = MDRNNCell(128, ACTION_SIZE, 512, 5)
    ...
    def step(self, action):
        ...
        with torch.no_grad():
            ...
            print(action)
            mu, sigma, pi, n_h = self._rnn(action, self._lstate, self._hstate)
            pi = pi.squeeze()
            ...
            self._obs = self.vae.decode(self._lstate)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, help='Directory from which MDRNN and VAE are retrieved.',
                        default="/home/ivkic_urise/Documents/FINAL_TRAINED/MDRNN_Phantom/MDRNN_5gmm_FINAL/",
                       )
